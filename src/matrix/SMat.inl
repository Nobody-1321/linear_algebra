...
namespace line
{
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SMat<R, C, T>::SMat(T val_t) : rows{std::make_unique<std::array<row_type, R>>()}
    {
        for (std::size_t i = 0; i < R; i++)
        {
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SMat<R, C, T>::SMat(const SMat<R, C, T> &mat_) : rows{std::make_unique<std::array<row_type, R>>()}
    {
        for (int i = 0; i < R; i++)
        {
            (*rows)[i] = mat_[i];
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SMat<R, C, T>::SMat(SMat<R, C, T> &&mat_) noexcept : rows{std::move(mat_.rows)}
    {
    }
    ...
    SMat<R, C, T>::SMat(Args &&...args) : rows{std::make_unique<std::array<row_type, R>>()}
    {
    ...
    // list initialization
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SMat<R, C, T>::SMat(std::initializer_list<std::initializer_list<T>> init_list) : rows{std::make_unique<std::array<row_type, R>>()}
    {
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SVec<R, T> SMat<R, C, T>::operator*(const SVec<R, T> &vec_) const
    {
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    SVec<R, T> &SMat<R, C, T>::operator*=(const SVec<R, T> &vec_)
    {
        static_assert(C == R, "Matrix dimensions do not match for multiplication");
    ...
    // access to elements
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    typename SMat<R, C, T>::row_type &SMat<R, C, T>::operator[](int idx) noexcept
    {
        return (*rows)[idx];
    }

    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    typename SMat<R, C, T>::row_type const &SMat<R, C, T>::operator[](int idx) const noexcept
    {
        return (*rows)[idx];
    }
    ...
    template <nsp_types::length_t R, nsp_types::length_t C, nsp_concepts::is_numeric T>
    typename SMat<R, C, T>::const_iterator SMat<R, C, T>::cbegin() const noexcept
    {
