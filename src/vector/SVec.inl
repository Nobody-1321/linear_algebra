namespace line
{
    ...
    SVec<L, T>::SVec() : data_v{std::make_unique<array_type>()}, is_empty_{false}
    {
        data_v->fill(0);
    }

    template <nsp_types::length_t L, nsp_concepts::is_numeric T>
    SVec<L, T>::SVec(const SVec<L, T> &vec_) : data_v{std::make_unique<array_type>()}, is_empty_{false}
    {
    ...
    SVec<L, T>::SVec(Args &&...args) : data_v{std::make_unique<array_type>()}, is_empty_{false}
    {
    ...
    // list initialization
    template <nsp_types::length_t L, nsp_concepts::is_numeric T>
    SVec<L, T>::SVec(std::initializer_list<T> init_list) : data_v{std::make_unique<array_type>()}, is_empty_{false}
    {
    ...
    template <nsp_types::length_t L, nsp_concepts::is_numeric T>
    bool SVec<L, T>::is_empty() const noexcept
    {
        return is_empty_;
    }
